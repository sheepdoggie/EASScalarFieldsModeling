# GitHub install and publish: v0.1.29

```bash
cd ~/Projects/EASScalarFieldsModeling_github_files
unzip EASScalarFieldsModeling_v0.1.29_github_package.zip
...
rsync -a --delete --exclude='.git' \
  ../EASScalarFieldsModeling_v0.1.29_github_package/ \
  ./
...
rank3-check-release-identity \
  ...
  --framework-zip releases/current/enforceable_rank3_modeling_v0.1.29_contract_overlay_synthesis.zip \
  --framework-tar-gz releases/current/enforceable_rank3_modeling_v0.1.29_contract_overlay_synthesis.tar.gz

git status --short
git add .
git commit -m "Package GitHub-installable framework v0.1.29"
git push origin main

git tag -a v0.1.29 -m "v0.1.29 contract-driven overlay synthesis"
...
python tools/verify_release_manifest.py \
  ...
  releases/current/enforceable_rank3_modeling_v0.1.29_contract_overlay_synthesis.zip
```
