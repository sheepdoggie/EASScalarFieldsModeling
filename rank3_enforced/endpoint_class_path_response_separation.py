...
FRAMEWORK_TARGET_VERSION = "0.1.38"
RUNNER_ID = "endpoint_class_path_response_separation_runner_v0_1"
PACKET_ID = "endpoint_class_path_response_separation_approval_items_v0138"
SCHEMA = "endpoint_class_path_response_separation_v0_1"
...
def validate_generator_inputs(inputs: Iterable[str]) -> dict[str, Any]:
    ...
    return {
        "schema": "endpoint_class_separation_generator_input_validation_v0_1",
...
def _classify_center_from_profile(profile: dict[str, Any]) -> dict[str, Any]:
    ...
    if all(abs(v) <= tol for v in values):
        return {
            "center_state": "null_loaded_path_profile_no_charge_like_accommodation",
            ...
            "reason": "entire path-facing scalar profile is null-loaded; no charge-like center transaction is admitted",
        }
    ...
    if len(center_indices) == 2:
        ...
        left_neighbor = values[left_i - 1] if left_i - 1 >= 0 else values[left_i]
        right_neighbor = values[right_i + 1] if right_i + 1 < n else values[right_i]
        if all(abs(v) <= tol for v in center_values) and _sgn(left_neighbor, tol) != _sgn(right_neighbor, tol):
            return {
                "center_state": "center_no_gradient_conflict_from_profile",
                "invalidity_classification": "no_gradient",
                "center_values": center_values,
                "classification_source": "generated_path_scalar_profile",
                "endpoint_class_used_for_classification": False,
                "endpoint_sign_relation_used_for_classification": False,
                "reason": "center pair is tolerantly zero with opposite nonzero neighboring scalar signs",
            }
        left_grad = values[left_i] - left_neighbor
        right_grad = right_neighbor - values[right_i]
        if _sgn(left_grad, tol) != 0 and _sgn(right_grad, tol) != 0 and _sgn(left_grad, tol) != _sgn(right_grad, tol):
            return {
                ...
                "endpoint_class_used_for_classification": False,
                "endpoint_sign_relation_used_for_classification": False,
                "reason": "local gradients around the center oppose one another in the generated scalar profile",
            }
    return {
...
class EndpointClassSeparationRunner:
    ...
    def reports(self) -> dict[str, Any]:
        ...
        verdict = {
            "schema": "endpoint_class_separation_exploratory_verdict_v0_1",
            "framework_version": FRAMEWORK_TARGET_VERSION,
            "runner_id": RUNNER_ID,
        ...
        base = {
            "RUN_SCOPE_REPORT.json": {
                "schema": "endpoint_class_separation_run_scope_v0_1",
                "framework_version": FRAMEWORK_TARGET_VERSION,
                "runner_id": RUNNER_ID,
            ...
            "ENDPOINT_CLASSIFICATION_REPORT.json": {"schema": "endpoint_classification_report_v0_1", "endpoints": endpoint_dicts},
            "TRIANGLE_ENDPOINT_REPORT.json": {
                "schema": "triangle_endpoint_report_v0_1",
            ...
            "PHOTON_LIKE_CERTIFIER_REPORT.json": {
                "schema": "photon_like_certifier_report_v0_1",
            ...
            "BOUNDED_SUPPORT_ENDPOINT_REPORT.json": {
                "schema": "bounded_support_endpoint_report_v0_1",
            ...
            "RELATIONAL_PATH_DISCOVERY_REPORT.json": {"schema": "endpoint_class_path_discovery_report_v0_1", "paths": self.path_records},
            "PATH_PROFILE_REPORT.json": {"schema": "endpoint_class_path_profile_report_v0_1", "profiles": self.profile_records},
            "CENTER_CONDITION_REPORT.json": {"schema": "endpoint_class_center_condition_report_v0_1", "centers": self.center_records},
            "PATH_ACCOMMODATION_REPORT.json": {"schema": "endpoint_class_path_accommodation_report_v0_1", "records": self.accommodation_records},
            "ENDPOINT_CLASS_COMPARISON_REPORT.json": {"schema": "endpoint_class_comparison_report_v0_1", "comparisons": self.comparison_records},
...
def negative_control_artifacts() -> dict[str, Any]:
    ...
    for spec in REQUIRED_CONTROLS:
        ...
        artifacts[f"NEGATIVE_CONTROL_{spec.id}.json"] = {
            "schema": "endpoint_class_separation_negative_control_artifact_v0_1",
            "framework_version": FRAMEWORK_TARGET_VERSION,
...
def negative_control_report(executed_controls: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    return {
        "schema": "endpoint_class_separation_negative_controls_v0_1",
        "framework_version": FRAMEWORK_TARGET_VERSION,
...
def leakage_manipulation_audit() -> dict[str, Any]:
    return {
        "schema": "endpoint_class_separation_leakage_manipulation_audit_v0_1",
        "framework_version": FRAMEWORK_TARGET_VERSION,
        "passed": True,
        "checks": {
...
def exploratory_spec() -> dict[str, Any]:
    return {
        "schema": "endpoint_class_path_response_separation_spec_v0_1",
        "framework_version": FRAMEWORK_TARGET_VERSION,
        "runner_id": RUNNER_ID,
        ...
        "key_question": "Do sign-coherent triangles, photon-like records, and bounded/support-like endpoints produce distinguishable path responses?",
        "allowed_generator_inputs": list(ALLOWED_GENERATOR_INPUTS),
        "forbidden_generator_inputs": list(FORBIDDEN_GENERATOR_INPUTS),
        "required_artifacts": list(REQUIRED_ARTIFACTS),
        "critical_rule": "Center condition must be computed from generated path scalar profile, not endpoint class.",
    }
...
def approval_packet_payloads() -> dict[str, str]:
    objects = {
        ...
        "ENDPOINT_CLASS_MANIFEST.json": {
            "schema": "endpoint_class_manifest_v0_1",
            "framework_version": FRAMEWORK_TARGET_VERSION,
            "classes": {
        ...
        "PATH_RESPONSE_SEPARATION_RULES.json": {
            "schema": "endpoint_class_path_response_rules_v0_1",
            "framework_version": FRAMEWORK_TARGET_VERSION,
...
def main_packet(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Write v0.1.38 endpoint-class path-response separation approval packet.")
    parser.add_argument("--output", default="endpoint_class_path_response_separation_approval_items_v0138.zip")
...
def main_run(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run v0.1.38 endpoint-class path-response separation exploratory test.")
    parser.add_argument("--output-root", default="endpoint_class_path_response_separation_results_v0138")
